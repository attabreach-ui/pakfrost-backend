...
router.get('/stats', async (req: Request, res: Response) => {
  try {
    ...
    const [
      ...
      prisma.stockMovement.aggregate({ where: { type: 'IN',  status: 'active', createdAt: { gte: today } }, _sum: { cartons: true } }),
      prisma.stockMovement.aggregate({ where: { type: 'OUT', status: 'active', createdAt: { gte: today } }, _sum: { cartons: true } }),
    ]);
  ...
  try {
    ...
    for (let i = days - 1; i >= 0; i--) {
      ...
      const [inn, out] = await Promise.all([
        prisma.stockMovement.aggregate({ where: { type: 'IN',  status: 'active', createdAt: { gte: start, lte: end } }, _sum: { cartons: true } }),
        prisma.stockMovement.aggregate({ where: { type: 'OUT', status: 'active', createdAt: { gte: start, lte: end } }, _sum: { cartons: true } }),
      ]);
      result.push({
